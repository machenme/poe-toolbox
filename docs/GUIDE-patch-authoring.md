...
`version` 决定补丁写入哪个 bundle：写了是 `PATCHED/<bundleName>_v<version>`，
**不写**就是 `PATCHED/<bundleName>`——同一补丁恒定落在同一个 bundle 里。

补丁状态判定全部基于文件内容，不靠版本号区分，所以：

- 不写 version 也能正常应用、还原、重复执行；改了补丁内容直接覆盖自己的 bundle 即可，
  不需要升版本（旧版本遗留下来的 `_vN` bundle 里的产物仍会被认成本补丁自己的东西，
  从带版本改成不带版本也不会被自己上一版挡住）。
- 想让两个版本的补丁在磁盘上并存（例如需要回滚到某一版产物），才写 version。
