...
| **词缀上色** | PoE2 | 词缀自定义上色：读取客户端全部词缀描述文件（约 1.9 万条），按「装备词缀 / 地图与图鉴 / 掉落容器与遗物 / 技能与天赋 / 其他」分组浏览与搜索（支持正则），可勾选批量上色，颜色用 RGBA 自定义并支持一键生成等级色阶；自动识别客户端语言（简体中文 / 繁体中文 / 英文）并只给该语言上色，一键应用为游戏补丁、随时恢复；方案可保存与分享，内置「官方原版」方案可一键清除全部词缀颜色 |
...
打开“特效补丁”页面时会直接显示每个内置补丁装没装，**已经装上的会自动勾选**，想还原直接点「彻底还原游戏客户端」即可。这份状态来自游戏目录里的 `Bundles2\backup\applied-patches.json`——补丁应用时由引擎顺手写下的一个小记录文件，打开页面读它就知道装了哪些，不用为了看状态再去开几个 GB 的索引。记录只是展示用的账本，不参与还原（还原仍然靠索引本身与 `backup\_.index.bin` 基线）；万一它与实际情况对不上（比如用启动器修复过客户端），点「高级维护」里的「刷新补丁状态」会重新打开索引核对一遍并纠正记录。
...
| 操作 | 做什么 | 影响面 |
|------|--------|--------|
| 启用特效补丁 / 彻底还原游戏客户端（界面主按钮） | 写入/撤销补丁引用 | 可逆，随时来回切；还原会把游戏彻底还原成官方原版（含删除补丁文件） |
...
“完全卸载”不碰游戏原有文件，这一点是刻意的：还原补丁会把被改过的原版文件（如 `data/balance/miscanimated.datc64`）的内容重定向进补丁 Bundle，那条索引记录是客户端读取该文件的唯一途径，删掉等于把文件从客户端里挖走。
...
UI 里点“自定义补丁”直接选 zip 即可（补丁来源会自动切到自定义补丁，不用先改下拉菜单）。引擎会把补丁包解压到临时目录再执行，24 小时后自动清理，不影响原 zip 文件。

### 整包替换型补丁（作者直接打包的索引 + Bundle）

有些作者不提供描述文件，而是把自己改好的 `_.index.bin` 与若干 `.bin`（Bundle）直接打包发布。这类 zip 直接选它、点“启用特效补丁”即可，引擎会自动识别：

1. 以包内 `_.index.bin` 所在的那层目录为根（常见是 `bundles2/`，多包一层也没关系），其余 `.bin` 按相对路径落到游戏索引目录；
2. **覆盖前先把游戏目录里的同名文件备份到 `Bundles2\backup\<补丁名>\`**（补丁名 = 补丁 ID，没有描述文件时取 zip 文件名）。索引本体若与原版基线 `backup\_.index.bin` 内容一致，则直接复用基线、不重复复制一遍；
3. 备份完成后才逐个覆盖同名文件，日志会逐条打印备份与覆盖结果。

点“彻底还原游戏客户端”会按备份清单还原：原版有的文件放回去，补丁新增的文件删掉。`backup\_.index.bin` 是原版基线的专用位置，逐补丁备份不会动它。这类补丁不走索引级操作，因此“完全卸载补丁”对它不适用。
...
| `<游戏数据同目录>/backup/_.index.bin` | 每个客户端的原始索引备份，选择游戏数据时自动创建一次（已存在则不重复复制） |
