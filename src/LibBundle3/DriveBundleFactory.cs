...
public class DriveBundleFactory : IBundleFactory {
	...
	/// </param>
	/// <param name="readOnly">Open bundle files in read-only mode</param>
	public DriveBundleFactory(string baseDirectory, bool readOnly = false) {
	...
	public virtual Bundle GetBundle(BundleRecord record) {
		return new(BaseDirectory + record.Path, record, ReadOnly);
	}
