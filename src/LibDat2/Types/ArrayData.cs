			...
			switch (TypeOfValue) {
				case "bool":
					fixed (bool* b = (object)Value as bool[])
						reader.BaseStream.ReadExactly(new Span<byte>(b, Value.Length));
					break;
				case "i8":
				...
				case "i16":
					fixed (short* b = (object)Value as short[])
						reader.BaseStream.ReadExactly(new Span<byte>(b, Value.Length * sizeof(short)));
					break;
				case "i32":
					fixed (int* b = (object)Value as int[])
						reader.BaseStream.ReadExactly(new Span<byte>(b, Value.Length * sizeof(int)));
					break;
				case "i64":
					fixed (long* b = (object)Value as long[])
						reader.BaseStream.ReadExactly(new Span<byte>(b, Value.Length * sizeof(long)));
					break;
				case "u8":
					reader.BaseStream.ReadExactly((object)Value as byte[]);
					break;
				case "u16":
					fixed (ushort* b = (object)Value as ushort[])
						reader.BaseStream.ReadExactly(new Span<byte>(b, Value.Length * sizeof(ushort)));
					break;
				case "u32":
					fixed (uint* b = (object)Value as uint[])
						reader.BaseStream.ReadExactly(new Span<byte>(b, Value.Length * sizeof(uint)));
					break;
				case "u64":
					fixed (ulong* b = (object)Value as ulong[])
						reader.BaseStream.ReadExactly(new Span<byte>(b, Value.Length * sizeof(ulong)));
					break;
				case "f32":
					fixed (float* b = (object)Value as float[])
						reader.BaseStream.ReadExactly(new Span<byte>(b, Value.Length * sizeof(float)));
					break;
				case "f64":
					fixed (double* b = (object)Value as double[])
						reader.BaseStream.ReadExactly(new Span<byte>(b, Value.Length * sizeof(double)));
					break;
				default:
